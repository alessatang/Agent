...
int convert_to_text(const char *buf, size_t *buf_len, const char *dir_full_path,  char *tmp_file_path)
{
	...
	DWORD dwFilesize = sizeof(text);

	if (!WriteFile(hTempFile, (LPVOID)text, dwFilesize, 0, NULL))
	{
